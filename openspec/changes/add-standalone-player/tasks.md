...
- [ ] Create `agent-backend/src/bundle/` directory
- [ ] Create `agent-backend/src/bundle/types.ts` with TypeScript interfaces:
...
- [ ] Create `agent-backend/src/bundle/__tests__/` directory for unit tests

### 1.2 Bundle Assembler Implementation
- [ ] Create `agent-backend/src/bundle/bundleAssembler.ts`
- [ ] Implement `assembleBundleJson(storyId, dependencies)` function
...
- [ ] Implement `determineNextState(scenelet, childScenelets, availableSceneletIds)` helper
...
- [ ] Write unit tests for bundleAssembler with complete and incomplete story scenarios

### 1.3 Asset Copier Implementation
- [ ] Create `agent-backend/src/bundle/assetCopier.ts`
- [ ] Implement `copyAssets(storyId, shots, outputPath)` function
...
- [ ] Implement `copyPlayerTemplate(templatePath, outputPath)` function
  - Copy player.html from template to `<outputPath>/<storyId>/player.html`
- [ ] Write unit tests for assetCopier with temp directories

### 1.4 Player Bundle Task Implementation
- [ ] Create `agent-backend/src/bundle/playerBundleTask.ts`
- [ ] Implement `runPlayerBundleTask(storyId, dependencies, options)` function
...
- [ ] Handle errors with descriptive messages (missing prerequisites, file I/O errors)
...
- [ ] Wire up CREATE_PLAYER_BUNDLE task in CLI task dispatcher
- [ ] Add help text for bundle task flags

### 2.3 Default Dependencies Wiring
- [ ] Create `agent-backend/src/bundle/index.ts` with default exports
- [ ] In CLI, instantiate default bundle assembler and asset copier
- [ ] Inject bundleAssembler and assetCopier into workflow options
- [ ] Set default template path to `agent-backend/src/bundle/templates/player.html`
...
- [ ] Create `agent-backend/src/bundle/templates/` directory
- [ ] Create `agent-backend/src/bundle/templates/player.html`
...
- [ ] Design CSS for player box layout
...
- [ ] Implement player state machine in vanilla JavaScript
...
- [ ] Implement `playAudio(audioPath)` function
  - Set audio element src to path
  - Handle audio ended event
  - Handle errors gracefully (missing file)
- [ ] Implement grace period timers with configurable delays

### 3.4 Auto-Play Linear Continuation
- [ ] Implement `advanceToNextScenelet(sceneletId)` function
  - Load scenelet from story data
  - Begin playing first shot
- [ ] Handle linear next.type: automatically call advanceToNextScenelet
- [ ] Ensure no user interaction required for linear flow

### 3.5 Choice UI Implementation
- [ ] Implement `displayChoiceUI(choicePrompt, choices)` function
...
- [ ] Implement `onChoiceSelected(sceneletId)` event handler
  - Hide choice overlay
  - Load selected scenelet and begin playback
- [ ] Style choice buttons for hover and click states

### 3.6 Terminal and Incomplete Ending UI
- [ ] Implement `displayTerminalUI()` function
  - Pause on last shot image
  - Display "Start Over" button with icon
- [ ] Implement `displayIncompleteUI()` function
  - Pause on last shot image
  - Do NOT display restart button (work-in-progress story)
  - Allow manual page reload to replay
- [ ] Implement `onRestartClicked()` event handler
  - Reset state to START
  - Display start screen
  - Clear playback history

### 3.7 Playback Controls
- [ ] Implement play/pause button toggle
...
- [ ] Position controls at bottom of player box
...
### 4.1 Backend Testing
- [ ] Write unit tests for bundleAssembler edge cases
...
- [ ] Add JSDoc comments to all public functions in bundle module
- [ ] Add TypeScript type annotations to all player.js functions
- [ ] Document player.html template structure

### 5.2 User Documentation
- [ ] Add README to `agent-backend/src/bundle/` explaining bundle structure
- [ ] Document CLI usage for CREATE_PLAYER_BUNDLE task
- [ ] Document expected output folder structure
...
- [ ] Run `openspec validate add-standalone-player --strict`
