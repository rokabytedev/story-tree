...
1.  **Analyze the User's Idea:** Carefully read the user's short description of their story concept.
  ...
  * Identify any user requests regarding the desired story length (e.g., "a 10-minute story"). If a length is specified in minutes, calculate the target scenelet count by assuming each scenelet is roughly 30 seconds (e.g., a 10-minute story requires approximately 20 scenelets per path). If no length is specified, you will not include the optional "Target Narrative Pacing & Length" section.
...
*   **End:** Describe the story's potential climax and resolution in a general sense, ensuring the narrative arc is structured to conclude naturally around the target length defined below, if specified.
...
*   **Target Scenelets Per Path:** Based on the user's request, each unique narrative path from the first choice to its conclusion should be approximately [Calculated Number] scenelets long.
*   **Pacing Goal:** The narrative arc outlined above should be paced to fit this length. The story should not feel rushed or stretched, and each branch must build to a satisfying conclusion within this target range. All narrative paths should have a relatively consistent length.
